/** Checks template refresh command invocation and failure handling. */
...
import { inspectImageDigest, refreshTemplates, resolveTemplateRefreshEntryTarget, templateRefreshDockerIsolationArgs } from "../src/template-refresh.js";
...
test("template refresh rejects an oversized local JAR before archive parsing", () => {
  const root = mkdtempSync(join(tmpdir(), "relution-template-refresh-oversized-"));
  const jarPath = join(root, "relution-exec.jar");
  const out = join(root, "template-bundle.json");
  writeFileSync(jarPath, "");
  truncateSync(jarPath, (512 * 1024 * 1024) + 1);

  assert.throws(
    () => refreshTemplates({ jar: jarPath, out, serverVersion: "test" }),
    /Template refresh JAR exceeds the 536870912 byte limit/u,
  );
  assert.equal(existsSync(out), false);
});

test("template refresh rejects a symlinked local JAR before archive parsing", () => {
  const root = mkdtempSync(join(tmpdir(), "relution-template-refresh-symlink-"));
  const target = join(root, "target.jar");
  const jarPath = join(root, "relution-exec.jar");
  const out = join(root, "template-bundle.json");
  writeMinimalRelutionJar(target);
  symlinkSync(target, jarPath);
...
test("template refresh rejects a non-regular local JAR before archive parsing", () => {
  const root = mkdtempSync(join(tmpdir(), "relution-template-refresh-directory-"));
  const jarPath = join(root, "relution-exec.jar");
  const out = join(root, "template-bundle.json");
  mkdirSync(jarPath);
  ...
  assert.throws(
    () => refreshTemplates({ jar: jarPath, out, serverVersion: "test" }),
    /Template refresh JAR must be a regular file/u,
  );
  assert.equal(existsSync(out), false);
});
...
test("template refresh containers run without network, capabilities, or writable root filesystems", () => {
  const base = templateRefreshDockerIsolationArgs(false);
  assert.equal(base.includes("--network=none"), true);
  assert.equal(base.includes("--cap-drop=ALL"), true);
  assert.equal(base.includes("--security-opt=no-new-privileges"), true);
  assert.equal(base.includes("--read-only"), true);
  assert.equal(base.some((argument) => argument.startsWith("--tmpfs")), false);

  const reflection = templateRefreshDockerIsolationArgs(true);
  assert.deepEqual(reflection.slice(-2), ["--tmpfs", "/tmp:rw,nosuid,nodev,noexec,size=64m"]);
});
