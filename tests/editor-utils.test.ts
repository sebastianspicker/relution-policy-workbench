/** Covers editor utility normalization and local API URL safety helpers. */
...
import { editorApiFetch } from "../web/src/editor/editor-api-client.js";
import { readJsonResponse } from "../web/src/editor/editor-record-utils.js";
import { deletePath, getPath, setPath } from "../web/src/editor/editor-object-path.js";
import { emptyObjectListRow } from "../web/src/editor/editor-object-list.js";
...
test("object paths reject prototype segments and preserve own nested values", () => {
  const record: Record<string, unknown> = {};

  setPath(record, "details.camera.enabled", true);
  assert.equal(getPath(record, "details.camera.enabled"), true);
  assert.equal(getPath(record, "__proto__.polluted"), undefined);
  assert.throws(() => setPath(record, "constructor.polluted", true), /Unsafe object path segment/u);

  deletePath(record, "details.camera.enabled");
  assert.equal(getPath(record, "details.camera.enabled"), undefined);
  assert.equal(({} as Record<string, unknown>).polluted, undefined);
});

test("object-list rows clone mutable defaults for each row", () => {
  const field = { itemFields: [{ id: "allowed", kind: "array" }, { path: "nested.enabled", kind: "boolean" }] };
  const first = emptyObjectListRow(field);
  const second = emptyObjectListRow(field);
  (first.allowed as unknown[]).push("first");

  assert.deepEqual(second, { allowed: [], nested: { enabled: false } });
});
