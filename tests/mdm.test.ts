/** Verifies deterministic MDM artifact generation, diffs, and manifest checks. */
...
test("offline validation and regeneration are deterministic", () => {
  ...
  const firstManifest = readFileSync(join(root, "mdm/generated/rexp-studio/manifest.json"), "utf8");
  const second = generateMdm(root);
  const secondManifest = readFileSync(join(root, "mdm/generated/rexp-studio/manifest.json"), "utf8");
...
test("MDM YAML loading rejects policy symlinks", () => {
  const root = mkdtempSync(join(tmpdir(), "mdm-yaml-symlink-"));
  write(root, "outside.yaml", "policy_id: outside\n");
  mkdirSync(join(root, "mdm/policies/apple"), { recursive: true });
  symlinkSync(join(root, "outside.yaml"), join(root, "mdm/policies/apple/linked.yaml"));

  assert.throws(() => loadMdmPolicySources(root), /MDM policy source must not use symlinks/u);
});

test("MDM source verification rejects cache-prefixed paths that escape the source cache", () => {
  const root = createFixture();
  write(root, "outside.pdf", "must not be considered evidence");
  assertUnsafeSourcePath(root, "private/source-pdfs-cache/../../outside.pdf");
});

test("MDM source verification reports a cache directory as unsafe instead of reading it", () => {
  const root = createFixture();
  assertUnsafeSourcePath(root, "private/source-pdfs-cache");
});
...
function createFixture(): string {
  ...
  writeJson(root, "mdm/evidence/source-manifest.json", {
    sources: [fixtureSource(pdfPath, createHash("sha256").update(pdf).digest("hex"))],
  });
...
function fixtureSource(local_path: string, sha256: string | null): Record<string, unknown> {
  return {
    id: "source",
    title: "Source",
    publisher: "BSI",
    ...
    licence: "fixture",
    scope: "fixture",
    extraction: { status: "extracted", pages: 1, text_sha256: "a".repeat(64), engine: "fixture" },
  };
}

function assertUnsafeSourcePath(root: string, localPath: string): void {
  writeJson(root, "mdm/evidence/source-manifest.json", { sources: [fixtureSource(localPath, null)] });
  const issues = verifyMdmSources(root);
  assert.deepEqual(issues.map((issue) => issue.message), ["source must be a regular file in the ignored PDF cache"]);
}
