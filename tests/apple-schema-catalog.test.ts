/** Validates Apple schema catalog loading, updates, and path-safety failures. */
...
test("refreshAppleSchemaCatalog derives stable labels from separator-heavy source names", async () => {
  const root = mkdtempSync(join(tmpdir(), "relution-apple-label-refresh-"));
  const profilesDir = join(root, "mdm", "profiles");
  mkdirSync(profilesDir, { recursive: true });
  writeFileSync(
    join(profilesDir, "com.apple.foo__bar.yaml"),
    [
      "payload:",
      "  payloadtype: com.example.label",
      "  supportedOS:",
      "    iOS:",
      "      introduced: '1.0'",
      "payloadkeys: []",
  ...
  const catalog = await refreshAppleSchemaCatalog({ source: root, revision: "test-fixture", out: join(root, "catalog.json") });
  assert.equal(catalog.entries[0]?.title, "Foo Bar");
});
