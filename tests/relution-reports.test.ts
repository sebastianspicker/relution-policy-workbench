/** Protects atomic report writing, markdown rendering, and evidence integrity. */
import assert from "node:assert/strict";
import { existsSync, lstatSync, mkdtempSync, mkdirSync, readFileSync, readdirSync, symlinkSync, truncateSync, unlinkSync, utimesSync, writeFileSync } from "node:fs";
...
test("Relution reports reject a symlinked workspace root", () => {
  const root = mkdtempSync(join(tmpdir(), "relution-reports-workspace-link-"));
  const workspace = join(root, "workspace");
  const workspaceLink = join(root, "workspace-link");
  mkdirSync(workspace);
  symlinkSync(workspace, workspaceLink);

  assert.throws(() => writeRelutionReport(workspaceLink, assessment()), /must not use symlinks/u);
});
...
test("Relution report history uses the embedded assessment time instead of mutable file metadata", () => {
  const workspace = temporaryWorkspace();
  const generatedAt = "2001-01-01T00:00:00.000Z";
  const files = writeRelutionReport(workspace, assessment(generatedAt));
  const future = new Date("2099-01-01T00:00:00.000Z");
  utimesSync(join(workspace, files.jsonPath), future, future);
  utimesSync(join(workspace, files.markdownPath), future, future);

  assert.equal(listRelutionReports(workspace)[0]?.generatedAt, generatedAt);
});

test("Relution report history does not read oversized JSON files into memory", () => {
  const workspace = temporaryWorkspace();
  const reportDir = join(workspace, "reports");
  const stem = "relution-compliance-report-00000000-0000-0000-0000-000000000000";
  mkdirSync(reportDir);
  const jsonFile = join(reportDir, `${stem}.json`);
  const markdownFile = join(reportDir, `${stem}.md`);
  writeFileSync(jsonFile, "{}");
  truncateSync(jsonFile, 17 * 1024 * 1024);
  writeFileSync(markdownFile, "# Large report\n");
  const modified = new Date("2002-01-01T00:00:00.000Z");
  utimesSync(jsonFile, modified, modified);

  const [entry] = listRelutionReports(workspace);
  assert.equal(entry?.sizeBytes, 17 * 1024 * 1024);
  assert.equal(entry?.generatedAt, modified.toISOString());
});
