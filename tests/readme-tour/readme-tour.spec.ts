/** Captures the documented desktop tour against the production editor bundle. */
...
} from "../e2e/editor-flow-helpers.js";
import { EDITOR_E2E_API_TOKEN } from "../e2e/playwright-config-helpers.js";

const outputDir = resolve("docs/readme-tour");
const archiveSecret = process.env.RELUTION_README_REXP_KEY ?? "playwright-local-archive-key";
...
test("generate README product tour screenshots", async ({ page }) => {
  ...
  await expect(page.locator(".toolbar-product-name")).toHaveText("REXP Studio");
  await capture(page, "01-editor-overview.png", [
    page.locator(".toolbar-product-name"),
    page.getByRole("button", { name: "Build archive" }),
  ]);
...
async function mockDashboardApi(page: Page): Promise<void> {
  ...
  await page.route("**/api/relution/devices/audit", async (route) => {
    ...
    await route.fulfill({
      contentType: "application/json",
      body: JSON.stringify({
        assessmentId: "readme-tour-assessment",
        query: {
          baseUrl: "https://relution.example.org",
        ...
        report: {
          ...
          baseUrl: "https://relution.example.org",
          completeness: { assessedCount: devices.length, total: devices.length, truncated: false, status: "complete" },
          summary: {
            totalDevices: 2,
            compliant: 1,
            issue: 1,
