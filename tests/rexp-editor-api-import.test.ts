/** Verifies editor import routes preserve valid REXP workspace contents. */
...
test("updates the editor archive passphrase for rebuilt archives", async () => {
...
test("validates an editor archive passphrase against the current output archive", async () => {
...
test("does not mark an editor archive passphrase validated when it cannot decrypt the current output archive", async () => {
  const { out, handle } = await startTestEditor({
...
test("blocks archive builds until an archive passphrase is set", async () => {
  ...
  try {
    ...
    assert.match(result.error ?? "", /archive passphrase/i);
  } finally {
    await handle.close();
  }
});
...
function listImportTempDirectories(): Set<string> {
  return new Set(readdirSync(tmpdir()).filter((name) => name.startsWith("rexp-studio-import-")));
}
