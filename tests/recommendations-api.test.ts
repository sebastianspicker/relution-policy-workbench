/** Checks recommendation API catalog, coverage, and validation response shapes. */
...
async function assertRecommendationSummaryApi(baseUrl: string, apiToken: string): Promise<void> {
  const response = await fetch(new URL("api/recommendations", baseUrl), { headers: { "x-rexp-studio-token": apiToken } });
  ...
  const summary = await response.json() as {
    sources: Array<{
      ...
      coverageSummary: { exactMappings: number; partialRecommendations: number };
    }>;
  };
...
async function assertVendorRecommendationApi(baseUrl: string, apiToken: string): Promise<void> {
  const response = await fetch(new URL("api/recommendations/vendor", baseUrl), { headers: { "x-rexp-studio-token": apiToken } });
  assert.equal(response.status, 200);
...
async function assertCisRecommendationApi(baseUrl: string, apiToken: string): Promise<void> {
  const response = await fetch(new URL("api/recommendations/cis", baseUrl), { headers: { "x-rexp-studio-token": apiToken } });
  assert.equal(response.status, 200);
  ...
  assert.equal(cis.recommendations.some((entry) => entry.platform === "MACOS" && entry.fallbackTranslations?.some((fallback) => fallback.method === "profile-method") === true), true);
  assert.equal(cis.recommendations.some((entry) => entry.platform === "WINDOWS" && entry.fallbackTranslations?.some((fallback) => fallback.method === "powershell" && fallback.commands.length > 0) === true), true);
...
async function assertRecommendationCoverageApi(baseUrl: string, apiToken: string): Promise<void> {
  const response = await fetch(new URL("api/recommendations/coverage", baseUrl), { headers: { "x-rexp-studio-token": apiToken } });
  assert.equal(response.status, 200);
  ...
  assert.equal(coverage.rows.some((row) => row.source === "cis" && row.recommendationId === "cis-apple-macos-15-sequoia-2-0-0-1-2" && row.category === "relution-achievable" && row.surfaces.includes("apple-schema-profile")), true);
...
async function assertRecommendationSemanticsApi(baseUrl: string, apiToken: string): Promise<void> {
  const response = await fetch(new URL("api/recommendations/semantics", baseUrl), { headers: { "x-rexp-studio-token": apiToken } });
  assert.equal(response.status, 200);
  ...
  assert.equal(semantics.relutionTargets.some((target) => target.target === "WINDOWS_ANTIVIRUS" && target.conceptIds.includes("malware_protection") && target.exactRecommendationIds.some((id) => id.startsWith("vendor:"))), true);
...
async function assertRecommendationSemanticAnalysisApi(baseUrl: string, apiToken: string): Promise<void> {
  const response = await fetch(new URL("api/recommendations/semantic-analysis", baseUrl), { headers: { "x-rexp-studio-token": apiToken } });
  assert.equal(response.status, 200);
  const semanticAnalysis = await response.json() as {
    precedence: { authoritativeSource: string; behavior: string };
    summary: { totalCommonGroups: number; sourceRecommendationCounts: Record<string, number> };
    commonGroups: Array<{ platform: string; conceptId: string; sources: string[] }>;
  ...
  assert.equal(semanticAnalysis.commonGroups.some((group) => group.platform === "IOS" && group.conceptId === "passcode_authentication" && group.sources.includes("bsi") && group.sources.includes("cis")), true);
...
async function assertMissingRecommendationSource(baseUrl: string, apiToken: string): Promise<void> {
  const missingResponse = await fetch(new URL("api/recommendations/not-a-source", baseUrl), { headers: { "x-rexp-studio-token": apiToken } });
