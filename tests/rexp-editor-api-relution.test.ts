/** Covers authenticated editor routes that proxy Relution assessment operations. */
...
test("editor Relution audit preserves prototype-named expected-policy platforms", async () => {
  const originalFetch = globalThis.fetch;
  const keys = ["__proto__", "constructor", "toString"];
  globalThis.fetch = async (input, init) => {
    const url = new URL(input instanceof Request ? input.url : input);
    if (url.hostname !== "relution.example.test") return originalFetch(input, init);
    return new Response(JSON.stringify({
  ...
  const root = mkdtempSync(join(tmpdir(), "relution-editor-api-platform-keys-"));
  const workspace = join(root, "workspace");
  let handle: Awaited<ReturnType<typeof startEditorServer>> | undefined;
  createNewWorkspace({
    workspace,
    platform: "IOS",
    name: "Relution platform key test",
    serverVersion: loadTemplateBundle().serverVersion,
  });
  try {
    handle = await startEditorServer({
    ...
    await postJson(handle.url, handle.apiToken, "/api/relution/session", {
      protocol: "https",
      host: "relution.example.test",
      apiToken: "secret-token",
    });
    const expectedPoliciesByPlatform = Object.fromEntries(keys.map((key) => [key, [`${key} baseline`]]));
    const audit = await postJson<{ report: { summary: { missingPolicy: number; byPlatform: Record<string, number> } } }>(
      handle.url,
      handle.apiToken,
    ...
    assert.equal(audit.report.summary.missingPolicy, keys.length);
    for (const key of keys) {
      assert.equal(audit.report.summary.byPlatform[key], 1);
    }
  } finally {
    await handle?.close();
    globalThis.fetch = originalFetch;
...
async function getJson<T>(baseUrl: string, apiToken: string, path: string): Promise<T> {
  const response = await fetch(localApiUrl(baseUrl, path), { headers: { "x-rexp-studio-token": apiToken } });
  const text = await response.text();
