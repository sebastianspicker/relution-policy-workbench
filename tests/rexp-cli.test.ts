/** Verifies REXP CLI commands, key handling, and user-facing error paths. */
...
test("serve creates a blank workspace and does not require an archive passphrase", () => {
...
test("serve package smoke works outside the repo cwd", () => {
  ...
  assert.match(result.stdout, /REXP Studio:/u, "package smoke should print server URL");
});
