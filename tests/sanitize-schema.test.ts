/** Validates schema sanitization before user-facing editor metadata is emitted. */
...
const TEST_POLICY_UUID = "SANITIZE-SCHEMA-POLICY";
const TEST_POLICY_NAME = "Sanitize Schema Policy";
const TEST_ARCHIVE_KEY = "sanitize-schema-test-key-2026";
...
function createWorkspace(details: Record<string, unknown>): PolicyWorkspace {
  return {
    metadata: {},
    report: {
      policiesToExport: [TEST_POLICY_UUID],
      exportedPolicies: {
        [TEST_POLICY_UUID]: {
          policyUuid: TEST_POLICY_UUID,
    ...
    policies: [
      {
        path: `policies/policy_${TEST_POLICY_UUID}.json`,
        document: {
          uuid: TEST_POLICY_UUID,
