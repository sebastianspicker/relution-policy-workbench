/** Protects core encrypted REXP packing, verification, extraction, and safety invariants. */
...
import { recordMobileConfigRestoreEntries, reconcileMobileConfigRestoreEntries } from "../src/sidecar.js";
...
test("rejects an incorrect archive passphrase", () => {
...
test("extractRexp rejects policy entries that escape the output root", () => {
  ...
  assert.throws(() => extractRexp(archive, outputDir, password), /outside the managed REXP format|outside extraction root/i);
...
test("verification rejects missing archive hashes", () => {
  ...
  assert.throws(() => verifyRexp(missingHash, password), /Archive hash map is missing metadata\.json/u);
});
...
import "./rexp-core-validation.test-cases.js";
